...
class LastReceiverMatchThreeYearsOrGreaterSpecificationSpec extends Specification {
    ...
    def "returns true if last match date greater than 3 years"() {
        ...
        def match = matches.get(6)
        store.getMatches(6) >> match
        specification = new LastReceiverMatchThreeYearsOrGreaterSpecification(store, match[0].giver)
    ...
    def "returns false when latest match is less than 3 years"(){
        ...
        specification = new LastReceiverMatchThreeYearsOrGreaterSpecification(store, match[2].giver)

        when:
        def actual = specification.isSatisfiedBy(match[2].receiver)
